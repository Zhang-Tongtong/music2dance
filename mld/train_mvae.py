...
class Trainer:
    def __init__(self, args: Args):
        ...
        elif data_args.dataset == 'finedance':  #改
            dataset_class = FineDanceDataset
        else:
    ...
    def calc_loss(self, motion, cond, history_motion, future_motion_gt, future_motion_pred, latent, dist):
        ...
        #         # 使用SMPL模型计算joints
        #         smplx_param = get_smplx_param_from_6d(smpl_param_dict)
        #         smplxout = body_model(return_verts=False, **smplx_param)
        #         smpl_joints.append(smplxout.joints[:, :22, :].reshape(sub_batch_size, num_frames, 22, 3))
        #         # joints.append(motion_dict['joints'].reshape(sub_batch_size, num_frames, 22, 3))
        ...
        def get_smpl_body(motion_tensor, genders, betas):
            """
            Args:
                motion_tensor: [B, T, 276]
                genders: list of gender strings
                betas: [B, T, 10]
            """
            batch_size, num_frames, feature_dim = motion_tensor.shape
            assert feature_dim == 276, f"Expected 276 features, got {feature_dim}"
            ...
            for gender_name in ['female', 'male']:
                ...
                # [sub_batch_size, num_frames, 276]
                gender_motion_tensor = motion_tensor[gender_idx, :, :]
                
                # 反归一化
                gender_motion_tensor = dataset.denormalize(gender_motion_tensor)  # [sub_batch_size, T, 276]
                ...
                # 'poses_6d': [sub_batch_size, T, 132]
                # 'joints': [sub_batch_size, T, 66]
                # etc.
                
                # Reshape为2D用于SMPL模型
                smpl_param_dict = {
                    'transl': motion_dict['transl'].reshape(sub_batch_size * num_frames, 3),
                    'poses_6d': motion_dict['poses_6d'].reshape(sub_batch_size * num_frames, 132),
                    'betas': gender_betas,
                    'gender': gender_name,
                }
                # print(f"DEBUG - smpl_param_dict keys: {smpl_param_dict.keys()}")
                # print(f"DEBUG - transl shape: {smpl_param_dict['transl'].shape}")
                # print(f"DEBUG - poses_6d shape: {smpl_param_dict['poses_6d'].shape}")
                # print(f"DEBUG - betas shape: {smpl_param_dict['betas'].shape}")
                
                # 提取预测的joints
                joints_tensor = motion_dict['joints'].reshape(sub_batch_size, num_frames, 22, 3)
                joints.append(joints_tensor)
                
                # 使用SMPL模型计算joints
                smplx_param = get_smplx_param_from_6d(smpl_param_dict)
                smplxout = body_model(return_verts=False, **smplx_param)
                smpl_joints_tensor = smplxout.joints[:, :22, :].reshape(sub_batch_size, num_frames, 22, 3)
                smpl_joints.append(smpl_joints_tensor)
